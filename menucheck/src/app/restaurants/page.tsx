...
export default function Restaurants(){
    ...
    useEffect(() => {
        async function getAllRestaurants() {
            try {
                ...
                const response = await apiService.get('restaurant'); // Chamada para o backend
                console.log(response);
                
                setRestaurants(response || []); // Armazena a lista de restaurantes no estado
            } catch (error) {
                console.error("Erro ao buscar restaurantes:", error);
            }
        }

        getAllRestaurants(); // Chama a função
    ...
    function calculateAverageStars(ratings: any) {
        ...
        const totalStars = ratings.reduce((sum: any, rating: any) => sum + rating.stars, 0);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
                    ...
                    {restaurants.map((restaurant) => (
                        ...
                        >
                            ...
                            <p className="text-gray-700">{restaurant.description || "Sem descrição disponível"}</p>
